...
Using Raspberry Pi, Open CV, and Tensorflow Lite, I was able to run and test an object detection model using machine learning in Python. The model can detect around 80 common objects including (cars, chairs, laptops, and other common objects). After finishing this, I was able to train and create my own machine-learning model using yolov8 through Python. I labeled my own images, and put them through the training process of the model, making sure to split up the dataset into training, validation, and testing datasets. I was able to get a relatively good accuracy within my model. After I finished creating my own model, I created another model that would detect license plates. The model would detect the license plate, crop the image, and then use an optical character recognition software known as easyocr to read the text within the license plate and output the text.
...
<img src="Hari-Project.jpg" width="500" height="400"> <img src="edgy.png" width="500" height="400">

# Final Project with modifications
I was able to train my own model by first creating and labeling 3 classes (Pickup truck, SUV, and Sedan). I first downloaded around 300 pictures from all these classes, separated the images into 3 different types, training, validating, and testing, and then used a tool called labelimg, to label my images. After I labeled my images using bounding boxes, I exported all the labels using the same name, using Yolo format, which gives the first value in the text file as the class name, and the next 4 values the same line as the values of the coordinates of the bounding boxes from a  range of 0-1. coordinates of the bounding box. After this, I uploaded all my images and labels into a Google Drive folder and created a Google colab. I made sure to keep the Google Collab images and Yolo files in the same folder. After this, I created a YAML file that stored all the names of the classes, and which yolo number corresponds to the class name. After this, I imported the YAML file into the folder with all the images and Yolo text files. Then using yoloV8 I was able to run the training, validation, and testing. In the beginning, the model wasn't working, but toward the end after changing a few directories, and a few code changes, I was able to get the model to relatively accurately predict what type of car different pictures are. 

I was also able to make a license plate detector using Python. There wasn't any training required to make the model. First, I installed easyocr and utils and then I imported various packages such as OpenCV, matplotlib, numpy, imutils, easyocr, and math. Then I imported the image and converted it into a grayscale. After this I created an if statement, to allow for the code to run through several loops if there is no text detected. I created 3 loops, one to change the values of the noise detection, one to change the first value of the edge detection, and one to change the second value of the edge detection. Then I found the different key points of the contours. Then I filtered the contours from largest to smallest and ran them through an if statement to see whether or not they had 4 key points or not. Then I store them within a variable to see if they are containing any text. Then we use easy ocr to check if there is any text within the variable. If there was text, it would break from the for loops and print the picture with the text and the result. If there was no text, it would continue and go through the loop until all the combinations of noise reduction and edge detection were run through.
