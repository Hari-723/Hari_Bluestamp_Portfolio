...
<img src="Hari-Project.jpg" width="500" height="400"> <img src="EDGEDFINAL.jpg.png" width="500" height="400">
...
I was also able to make a license plate detector using Python. There wasn't any training required to make the model. First, I installed easyocr and utils and then I imported various packages such as OpenCV, matplotlib, numpy, imutils, easyocr, and math. Then I imported the image and converted it into a grayscale. After this I created an if statement, to allow for the code to run through several loops if there is no text detected. I created 3 loops, one to change the values of the noise detection, one to change the first value of the edge detection, and one to change the second value of the edge detection. After this, I used different counters to change the values of the edge detection and noise reduction. Then I found the different key points of the contours. Then I filtered the contours from largest to smallest and then ran them through an if statement to see whether or not they had 4 key points or not. Then I store them within a variable to see if they are containing any text. Then we use easy ocr to check if there is any text within the variable. If there was text, it would break from the for loops and print the picture with the text and the result. If there was no text, it would continue and go through the loop until all the combinations of noise reduction and edge detection were run through.
